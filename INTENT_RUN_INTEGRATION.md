...
## 2. 最重要的一點:你的存取金鑰**沒有**寫進程式碼

這個分支(`claude/asciflow-door-module-locm0t`)**每次 push 都會自動部署到公開的 GitHub Pages 網址**(`.github/workflows/pages.yaml` 設定的)。如果把你的 MCP 端點網址(裡面含 `accessKey=qwe12326`)直接寫死在程式碼裡,等於是把這把金鑰**公開發布給任何造訪這個網址的人**——任何人打開瀏覽器的開發者工具、看一下網頁原始碼或打包後的 JS,就能拿到這把鑰匙,進而呼叫 `intent_action_run` 對你的 Google Sheets/Apps Script 專案做任何 IntentAction 允許的操作(寫入表格、切換 ClawBrain、刪除設定等)。

所以這次的做法是:**對話框裡有一個「MCP endpoint」輸入框,你自己貼上你的端點網址(含金鑰),這個值只存在你自己瀏覽器的 localStorage 裡**——跟其他 ASCIIFlow 的個人化設定(深色模式、格線顯示…)存放方式完全一樣,**不會被寫進 git 提交紀錄,也不會出現在部署到公開網址的 JS 檔案裡**。下次打開瀏覽器,ASCIIFlow 會記得你上次貼過的網址,不用重貼。
